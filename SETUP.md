...
Connect this repository to Vercel and expose it on `studio.moretes.com`.

## 1. Vercel project

https://vercel.com → **Add New… → Project** → import
`fernandofatech/aws-architecture-studio`. Framework: **Next.js**. Root
directory: `frontend`. Click **Deploy**.
...
Vercel: **Settings → Domains → Add** → `studio.moretes.com`.

## 5. Cloudflare DNS

Add CNAME `studio` → `cname.vercel-dns.com`, **DNS only** (gray cloud).

## 6. Verify

```bash
curl -I https://studio.moretes.com
```
